...
public class Trie {
	...
	private class TrieNode { // This is the class that represents a node in the trie
		...
		public int countAllWords() {
			//Declaring variable for storing the number of words in a trie 
			int numberOfWords = 0;
			//Running for loop for all the letters in the array
			for(int i=0;i<subnodes.length;i++)
			{
				//Checking all the values if that position of the array is null or not  
				if(subnodes[i]!=null)
				{
					//If it is not null then calling the
					//same function with that as array for next letter and 
					//adding the returned result in the variable
					numberOfWords +=  subnodes[i].countAllWords();
					//Checking if the valid end is true or not
					//If it is true it means it is the end of the word
					if(subnodes[i].isValidEnd==true)
					{
						//So, we will add one in the current number of the words and return it 
